...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/onboarding") {
      ...
      const plan = await orchestrate(profile, context, getSession(userId));
    ...
    if (req.method === "POST" && url.pathname === "/api/session-plan") {
      ...
      const plan = await orchestrate(profile, context, getSession(body.userId));
    ...
    if (req.method === "POST" && url.pathname === "/api/next") {
      ...
      const context = detectContext(body.signals || {});
      const profile = getProfile(body.userId);
      const plan = await nextBatch(profile, context, getSession(body.userId), body.n || 3);
      return send(res, 200, { context, ...plan });
    }

    if (req.method === "POST" && url.pathname === "/api/events") {
      const { userId, itemId, type, itemType, itemTopic } = await readJson(req);
